...
def check_syncing():
    ...
    response = requests.post('http://localhost:8545/route', data='{"id":1,"jsonrpc":"2.0","method":"eth_syncing","params":null}', headers={'Content-Type': 'application/json'})
...
def check_fcu_valid():
    ...
    response = requests.post('http://localhost:8545/route', data=ujson.dumps({'method': 'engine_forkchoiceUpdatedV1'}), headers={'Content-Type': 'application/json'})
